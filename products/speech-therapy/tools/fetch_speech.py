...
# Знак ударения U+0301 у голоса ru-RU-SvetlanaNeural отрабатывает НЕНАДЁЖНО (на слух
# ударение неверное почти везде). Поэтому полагаемся на РОДНОЙ лексикон Azure ru-RU
# (без знака — для обычных слов он ставит ударение правильно), а явный знак оставляем
# ТОЛЬКО для омографов, где значение зависит от ударения.
_STRESS_RU_RAW = {"замок": "замо+к"}   # 🔒 → замо́к (а не за́мок-крепость)
