...
def _legacy_secret(product_id: int):
    """Секрет HMAC для короткого ключа — свой на каждый продукт.

    Берётся из мастер-ключа экосистемы (ecosystem.keys, содержимое content_master):
        secret = HMAC-SHA256(master, b"legacy20:<product_id>")[:32]
    Тот же вывод даёт keygen_legacy20.py, поэтому ключи сходятся без обмена файлами.
    Возвращает list[int] или None, если мастер-ключ недоступен.
    """
    import hashlib
    import hmac as _hmac

    master_hex = os.environ.get("OI_CONTENT_MASTER", "").strip()
    if not master_hex:
        keys_path = CORE_ROOT / "ecosystem.keys"
        if not keys_path.exists():
            keys_path = CORE_ROOT.parent / "product-core" / "ecosystem.keys"
        if keys_path.exists():
            try:
                master_hex = json.loads(keys_path.read_text(encoding="utf-8"))["content_master"]
            except Exception as e:  # noqa
                log(f"⚠ ecosystem.keys не прочитан: {e}")
    if not master_hex:
        return None
    master = bytes.fromhex(master_hex.strip())
    return list(_hmac.new(master, f"legacy20:{product_id}".encode(), hashlib.sha256).digest())
...
def build_android(product_dir: str, debug: bool = False):
    ...
    for v in variants:
        ...
        if content_src.exists():
            ...
            # LEGACY-20: контент НЕ шифруется. Короткий ключ физически не может
            # нести ключ шифрования (20 символов), поэтому в этой схеме его
            # пришлось бы класть в APK — что защиты всё равно не даёт.
            log("LEGACY-20: контент не шифруется (короткий ключ, защита только от запуска)")
        ...
        # product_config.json (LEGACY-20: секрет HMAC для короткого ключа)
        product_config = {
        ...
        secret = _legacy_secret(cfg.get("product_id", 0))
        if secret:
            product_config["hmac_secret"] = secret
            log(f"LEGACY-20: секрет подписи встроен (продукт {cfg.get('product_id', 0)}, вариант {vid})")
        else:
            log("⚠ мастер-ключ не найден — активация работать не будет "
                "(нужен ecosystem.keys или переменная OI_CONTENT_MASTER)")
